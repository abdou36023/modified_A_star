function [im,disImg,dis] = aStar(startPoint,endPoint,img,showSteps,useSpeed,factor,resizeFactor)
mapImg = img(:,:,1);
dis = 0;
if useSpeed ==1
...
im = zeros(sz);
img = double(img>0)-1;
disToEnd = sqrt((startPoint(1)-149)^2+(startPoint(2)-338)^2);
img(startPoint(2),startPoint(1)) = 1e-6;
disImg(startPoint(2),startPoint(1)) = 1e-6;
open = [startPoint(2),startPoint(1),0,disToEnd,disToEnd];
disp(endPoint);
    ...
    for i = -1:1
        for j = -1:1
            ...
            if (x>0) &&(y>0) &&(x<sz(1)) && (y<sz(2)) && (img(x,y)==0)
                ...
                dte = sqrt((x-endPoint(2))^2+(y-endPoint(1))^2);
                if useSpeed == 1
                    param = (dfs+dte*factor)/obsticle(x,y);
                else 
                    param = (dfs+dte*factor);
                end
                open = [open;x,y,dfs,dte,param];
